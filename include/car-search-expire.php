...
if(isset($_GET['search'])){
  ...
  $qury=mysqli_query($project , "SELECT * FROM `car` WHERE (`car-color` LIKE '%$data%' or `car-num` = '$data' or `car-company-model`='$data' or `car-city`='$data') AND `car-expire-date` <= now()");
  if(mysqli_num_rows($qury) != 0){?>
    <div class="col-lg-12 m-1 col-sm-12 justify-content-center">
      <a href="car-expire.php" class="btn btn-success text-light justify-content-center w-100" >
      <h5>Refresh</h5>
      </a>
      </div>
...
<div class="text-center col-lg-3 col-sm-6 raduis-10 card m-2 border-0 p-3 shadow-sm <?php if($row['car-expire-date']<=date("Y-m-d")){echo 'bg-danger text-light';}else{echo 'bg-light';}?> ">
...
?>
  ...
  <?php }else{ ?>
   ...
   <a href="car-expire.php" class="btn btn-success text-light p-2 justify-content-center w-100" >
   <h5>Refresh</h5>
   </a>
